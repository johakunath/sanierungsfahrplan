...
const DetailRow = ({ label, value, bg }) => (
  <tr style={{ borderTop: "1px solid #E2DBD0", background: bg || "#FFFFFF" }}>
    <td style={{ padding: "9px 12px", width: 170, verticalAlign: "top", color: "#6B6259", fontWeight: 500, fontSize: 11 }}>{label}</td>
    <td style={{ padding: "9px 12px", verticalAlign: "top", color: "#1E1A15", fontSize: 11.5, lineHeight: 1.45 }}>{value}</td>
  </tr>
);
...
const ISFPPrintReport = ({ ist, k, heizkostenIst, aktivePakete, gebaeude, kumuliert }) => {
  ...
  const textColorFor = (klasse) => ["A+","A","B","C","D"].includes(klasse) ? "#1E1A15" : "#FFF";

  return (
    <div className="print-only" style={{ fontFamily: "'Geist', sans-serif", color: "#1E1A15" }}>
      {/* ═══ COVER / OVERVIEW PAGE ═══ */}
      <div style={{ padding: "20px 24px", pageBreakAfter: "always" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
          <div style={{ fontSize: 10, letterSpacing: "0.22em", color: "#6B6259" }}>01</div>
        ...
        <div style={{ height: 2, background: "#1E1A15", marginBottom: 14 }} />

        <div style={{ marginBottom: 4 }}>
          <span style={{ fontSize: 22, fontWeight: 500, fontFamily: "'Fraunces', serif" }}>ÜBERBLICK</span>
          <span style={{ fontSize: 11, color: "#6B6259", marginLeft: 10 }}>(Alle Zahlen nur beispielhaft — kein offizieller BAFA-iSFP)</span>
        </div>
        <div style={{ fontSize: 12, color: "#3A332B", marginBottom: 14 }}>
          {gebaeude.strasse}, {gebaeude.plz} {gebaeude.standort} · Baujahr {gebaeude.baujahr} · {fmt(gebaeude.wohnflaeche)} m² Wohnfläche
        </div>

        {/* HEUTE */}
        <div style={{ background: EFFIZIENZ_FARBEN[istKlasse] || "#E30613", color: textColorFor(istKlasse), padding: "14px 18px", display: "flex", gap: 18, alignItems: "center", marginBottom: 4 }}>
          <div style={{ fontSize: 10, letterSpacing: "0.3em", writingMode: "vertical-rl", transform: "rotate(180deg)", opacity: 0.85 }}>HEUTE</div>
          <div style={{ background: "#FFF", color: EFFIZIENZ_FARBEN[istKlasse], width: 56, height: 56, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 600, fontFamily: "'Fraunces', serif", borderRadius: 3 }}>{istKlasse}</div>
          <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 24px" }}>
            <div style={{ fontSize: 11, opacity: 0.9 }}>Jährliche Energiekosten</div>
            <div style={{ fontSize: 11, opacity: 0.9 }}>Jährliche CO₂-Emissionen</div>
            <div style={{ fontSize: 17, fontFamily: "'Geist Mono', monospace", fontWeight: 500 }}>{fmt(heizkostenIst)} €</div>
            <div style={{ fontSize: 17, fontFamily: "'Geist Mono', monospace", fontWeight: 500 }}>{fmt(co2Gesamt)} kg</div>
          </div>
        </div>

        <div style={{ fontSize: 12, color: "#3A332B", margin: "14px 0 10px", lineHeight: 1.5 }}>
          <b>Schrittweise Sanierung</b> — wir empfehlen folgende {aktivePaketeObj.length} Maßnahmenpakete in dieser Reihenfolge. Auf den Folgeseiten: Details pro Schritt.
        </div>
        ...
        {aktivePaketeObj.map((paket, i) => {
          ...
          const summeFoerder = paket.massnahmen.reduce((s, m) => {
            const netto = m.investition - m.ohnehin_anteil;
            const quote = m.foerderquote > 0 ? Math.min(m.foerderquote + BEG_BONUS.isfp_bonus, 0.5) : 0;
            return s + netto * quote;
          }, 0);
          return (
            <div key={paket.id} style={{ display: "flex", alignItems: "stretch", marginBottom: 4, minHeight: 52 }}>
              <div style={{ background: farbe.bg, color: farbe.text, width: 52, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <div style={{ fontSize: 20, fontWeight: 600, fontFamily: "'Fraunces', serif", lineHeight: 1 }}>{paket.nummer}</div>
                <div style={{ fontSize: 8, letterSpacing: "0.1em", marginTop: 2, opacity: 0.85 }}>▼</div>
              </div>
              <div style={{ width: 54, background: farbe.hell, display: "flex", alignItems: "center", justifyContent: "center", borderTop: "1px solid #D3CAB9", borderBottom: "1px solid #D3CAB9" }}>
                <div style={{ background: EFFIZIENZ_FARBEN[nachherKlasse], color: textColorFor(nachherKlasse), width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 600, fontFamily: "'Fraunces', serif", borderRadius: 2 }}>{nachherKlasse}</div>
              </div>
              <div style={{ flex: 1, padding: "8px 12px", borderTop: "1px solid #D3CAB9", borderBottom: "1px solid #D3CAB9", background: "#FFF" }}>
                <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 1 }}>{paket.titel}</div>
                <div style={{ fontSize: 10, color: "#6B6259", lineHeight: 1.35 }}>{paket.massnahmen.map(m => m.titel).join(" · ")}</div>
              </div>
              <div style={{ width: 250, fontSize: 10, padding: "7px 12px", borderTop: "1px solid #D3CAB9", borderBottom: "1px solid #D3CAB9", borderRight: "1px solid #D3CAB9", background: "#FFF", fontFamily: "'Geist Mono', monospace" }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ color: "#6B6259" }}>Invest. / davon förderf.</span><span>{fmt(summeInvest)} € / {fmt(summeFoerderfaehig)} €</span></div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "#00843D", marginTop: 2 }}><span>Förderung</span><span>{fmt(summeFoerder)} €</span></div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "#6B6259", marginTop: 2 }}><span>Zeitraum</span><span style={{ fontFamily: "'Geist', sans-serif" }}>{paket.zeitraum}</span></div>
              </div>
            </div>
          );
        })}
        ...
        {/* ZIEL */}
        <div style={{ background: EFFIZIENZ_FARBEN[k.effizienzklasse] || "#00843D", color: textColorFor(k.effizienzklasse), padding: "14px 18px", display: "flex", gap: 18, alignItems: "center", marginTop: 8 }}>
          <div style={{ fontSize: 10, letterSpacing: "0.3em", writingMode: "vertical-rl", transform: "rotate(180deg)", opacity: 0.85 }}>ZIEL</div>
          <div style={{ background: "#FFF", color: EFFIZIENZ_FARBEN[k.effizienzklasse], width: 56, height: 56, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 600, fontFamily: "'Fraunces', serif", borderRadius: 3 }}>{k.effizienzklasse}</div>
          <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 24px" }}>
            <div style={{ fontSize: 11, opacity: 0.9 }}>Jährliche Energiekosten (Ziel)</div>
            <div style={{ fontSize: 11, opacity: 0.9 }}>Jährliche CO₂-Emissionen (Ziel)</div>
            <div style={{ fontSize: 17, fontFamily: "'Geist Mono', monospace", fontWeight: 500 }}>{fmt(k.heizkosten_gesamt)} €</div>
            <div style={{ fontSize: 17, fontFamily: "'Geist Mono', monospace", fontWeight: 500 }}>{fmt(co2Ziel)} kg</div>
          </div>
        </div>

        <div style={{ fontSize: 9, color: "#6B6259", marginTop: 16, fontStyle: "italic", lineHeight: 1.5 }}>
          Demonstrator — kein offizieller BAFA-iSFP. Berechnungen orientieren sich an TABULA-Baseline und BEG 2026. Für den rechtsverbindlichen iSFP ist ein gelisteter Energieeffizienz-Experte erforderlich.
        </div>
      </div>
      ...
      {aktivePaketeObj.map((paket, i) => {
        ...
        return (
          <div key={paket.id} style={{ padding: "20px 24px", pageBreakBefore: "always" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
              <div style={{ fontSize: 10, letterSpacing: "0.22em", color: "#6B6259" }}>{String(i + 2).padStart(2, "0")}</div>
              <div style={{ fontSize: 10, letterSpacing: "0.22em", color: "#B5623E", fontFamily: "'Geist Mono', monospace" }}>iSFP-SCHNELLCHECK · DETAIL</div>
            </div>
            <div style={{ height: 1, background: "#D3CAB9", marginBottom: 14 }} />

            <div style={{ fontSize: 22, fontWeight: 500, fontFamily: "'Fraunces', serif", marginBottom: 12 }}>Schritt {paket.nummer} — {paket.titel}</div>

            <div style={{ display: "flex", alignItems: "stretch", marginBottom: 0 }}>
              <div style={{ background: farbe.bg, color: farbe.text, width: 52, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 600, fontFamily: "'Fraunces', serif" }}>{paket.nummer}</div>
              <div style={{ flex: 1, background: farbe.bg, color: farbe.text, padding: "10px 16px", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center" }}>{paket.titel}</div>
              <div style={{ background: farbe.bg, padding: "6px 12px", display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ background: EFFIZIENZ_FARBEN[nachherKlasse], color: textColorFor(nachherKlasse), width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 600, fontFamily: "'Fraunces', serif", borderRadius: 2 }}>{nachherKlasse}</div>
              </div>
            </div>

            <ul style={{ listStyle: "none", padding: "12px 16px", margin: 0, background: farbe.hell, fontSize: 11.5, lineHeight: 1.55 }}>
              {paket.massnahmen.map(m => (
                <li key={m.id} style={{ marginBottom: 5, paddingLeft: 14, position: "relative" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 10, fontSize: 11 }}>
              <tbody>
                <DetailRow label="Empfohlener Zeitraum" value={paket.zeitraum} bg="#F8F5EF" />
                <DetailRow label="Energiebedarf (nach Schritt)" value={
                  <span>Primärenergie: <b>{step.nachher.primaerenergie} kWh/(m²·a)</b><span style={{ marginLeft: 28 }}>Endenergie: <b>{step.nachher.endenergie} kWh/(m²·a)</b></span><span style={{ marginLeft: 28, color: "#6B6259" }}>Klasse: <b>{nachherKlasse}</b></span></span>
                } />
                <DetailRow label="Investition / davon förderfähig" value={
                  <span style={{ fontFamily: "'Geist Mono', monospace" }}>{fmt(summeInvest)} € / {fmt(summeFoerderfaehig)} €</span>
                } bg="#F8F5EF" />
                <DetailRow label="Förderung (inkl. iSFP-Bonus 5 %)" value={
                  <span style={{ fontFamily: "'Geist Mono', monospace", color: "#00843D" }}>− {fmt(summeFoerder)} € · {paket.massnahmen.map(m => `${m.foerderung_rechtsgrundlage} (${m.foerderung_stelle})`).filter((v, idx, arr) => arr.indexOf(v) === idx).join(" · ")}</span>
                } />
                <DetailRow label="Ihr Kostenanteil" value={
                  <span style={{ fontFamily: "'Geist Mono', monospace", fontWeight: 600 }}>{fmt(summeInvest - summeFoerder)} €</span>
                } bg="#F8F5EF" />
                <DetailRow label="Begründung" value={paket.begruendung} />
                <DetailRow label="Kosten-Herleitung" value={
                  <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
                    {paket.massnahmen.map(m => (
                      <li key={m.id} style={{ marginBottom: 3 }}><b>{m.titel}:</b> {m.kostenherleitung}</li>
                    ))}
                  </ul>
                } bg="#F8F5EF" />
                <DetailRow label="CO₂-Einsparung" value={
                  <span style={{ fontFamily: "'Geist Mono', monospace" }}>−{paket.massnahmen.reduce((s, m) => s + m.co2_reduktion, 0).toFixed(1)} kg/(m²·a)</span>
                } />
              </tbody>
            </table>
          </div>
